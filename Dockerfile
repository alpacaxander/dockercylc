...
RUN mkdir -p /opt /cylc-run/helloworld
...
RUN cylc register helloworld ./helloworld/ #&& cp /cylctest/helloworld/suite.rc /cylc-run/helloworld/

RUN \
	printf "[hosts]\n" > /opt/cylc-7.7.0/etc/global.rc ; \ 
	printf "[[localhost]]\n" >> /opt/cylc-7.7.0/etc/global.rc ; \
	printf "run directory = /cylc-run\n" >> /opt/cylc-7.7.0/etc/global.rc ; \
	printf "work directory = /cylc-run" >> /opt/cylc-7.7.0/etc/global.rc
